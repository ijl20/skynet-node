...
#define INTERRUPT_CHILD "interrupt_child"
  ...
  *cont = 1;
    ...
    if (strcmp(command,SET_VAR) == 0)
    ...
    else if (strcmp(command,START_PROG) == 0)
      {
        ...
        else if (start_prog(buffer_in + strlen(START_PROG)) == 0)
          { /* sprintf(buffer_out,"%s%s",OK,buffer_in);
               send_msg(buffer_out) */;
          }
        else
    ...
    else if (strcmp(command,INTERRUPT_CHILD) == 0)
      {
  ...
  if (sig == SIGCHLD)
    {
      printf("ppc: SIGCHLD\n");
      fflush(stdout);
      while (get_child()) {};        /* flush read pipe from child */
      /* child_running = 0; */
      close(TO_CHILD);
      while (wait3(&status, WNOHANG, 0) > 0) /* reap child */
        ;
      sprintf(buffer_out,"%schild exitted\n",OK);
      send_msg(buffer_out);
    }
