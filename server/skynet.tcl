...
# Note comments include reference to values G N L S C:
# G: number of path processors included in current group
# N: path processor id within current group, i.e. 0..G-1
# L: current depth limit for breadth-first search
# S: number of open ports found at depth L
...
#                           sky_kappa_running(i) = "depth host"
#
...
# kappa proc will send "delphi kappa started N G L"
...
proc kappa_started {host sock_data} {
  ...
  # add { host L } to sky_kappa_running list
...
# kappa proc will send "delphi kappa oracle N G L S [Orc]"
...
#  puts "debug: received oracle from <$host>"
  ...
  # note this 'add_running' is based on the assumption that the split processor will 
  # continue as one of the ppc's in the new group
  add_running $host $sky_host_l($host)
  
  # HERE IS WHERE THE WORK OF THE SPLIT PROC IS RE-ASSIGNED
...
# kappa proc will send "delphi kappa completed N G L S C"

# set host_status to WAITING
# remove host from 'running' list

# call kappa_split_test to find kappa_split_g waiting hosts
# and if that succeeds, call kappa_split
...
# manages the 'sky_kappa_running' list of running hosts, ordered by depth
# called from kappa_started() and kappa_oracle()

# e.g. sky_kappa_running = { {3 maxey mepal} {5 thriplow} ..}
...
# manages 'sky_kappa_running' global

# returns the hostname of a running host from the 'sky_kappa_running' list
# which is "depth host..." e.g. { {3 mepal maxey} {5 thriplow}...}

# sky_kappa_running is list ordered by depth, shallowest first
# hostnames in each sky_kappa_running list element are ordered by runtime, oldest first

proc get_running {} {
  global sky_kappa_running
  if {$sky_kappa_running == {}} { return {}
  } else {
    # pick host list at lowest depth e.g. 
    set current_hosts [lindex $sky_kappa_running 0]
    # current_hosts = { 3 mepal maxey }
    ...
    # new_hosts = { 3 maxey mepal }, so mepal has been moved to end (i.e. as newest)
    set sky_kappa_running [lreplace $sky_kappa_running 0 0 $new_hosts]
    # sky_kappa_running now has updated first element
    return $host
  }
}
...
# choose a host to 'split', and interrupt it
...
# Re-assign work to STATUS_PRIMED ppc's

# called when "delphi kappa oracle N G L S [Orc]" received from ppc
# N = ppc id of interrupted ppc
# G = ppc group count
# L = depth limit of interrupted ppc
# S = number of ports found at depth L
# [Orc] = oracle at time of interrupt e.g. [1,2,1,4,5,2,1,3,4,5]
...
# reset STATUS_PRIMED hosts back to STATUS_WAITING
